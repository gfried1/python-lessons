...
class BattleshipGame(object):
	...
	def boat_selection(self):
		...
		for counter in range(4):
			...
			message = "type the coordinates you want your boat to go (capitol letter than number no spaces for the coordinates) " + str(boat_length) + " this is the length of your boat"
			spot = self.choose_spot(message)
			self.boat = Boat(boat_length, spot[0], spot[1])
			self.boats.append(self.boat)

			# draw boat board
			self.board.draw_boats(self.boats)
	...
	def take_a_turn(self):	
		message = "write the place you want to hit (capitol letter than number no spaces). Type 'end' to quit."
		spot = self.choose_spot(message)
		if spot is None:
			return None

		self.board.mark_spot(spot[0], spot[1])
